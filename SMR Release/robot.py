...
class MyRobot(MagicRobot):
    ...
    def createObjects(self):
        ...
        self.grabber_motor = GrabberSparkMax(
            12, [], wheel_diameter=0.0762, gear_ratio=64 / 1
        )
        self.elevator_motor = ElevatorSparkMax(
            13, [], wheel_diameter=0.0382, gear_ratio=10 / 1
        )

        """Drivetrain Motor Configuration"""
        self.mainLeft_motor = ComboSparkMax(
            2,
            [1],
            inverted=False,
        ...
        self.mainRight_motor = ComboSparkMax(
            3,
            [4],
            inverted=True,
            wheel_diameter=0.1524,
    ...
    def teleopPeriodic(self) -> None:
        ...
        if self.hmi.getButton("Start"):  #! Resets superstructure to ground state
            self.grabber.openGrabber()
            if self.elevator.getDistance() >= 0.3:
                print(self.elevator.getDistance(), self.grabber.getDistance())
                self.grabber.goToLevel(3)
            self.elevator.goToLevel(0)

        if self.hmi.getButton(
            "Back"
        ):  #! Resets superstructure to ground state and disable brake.
